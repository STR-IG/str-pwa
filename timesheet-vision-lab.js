    ...
    try {
      ...
      if (!response.ok) {
        if (data?.error === 'OPENAI_API_KEY_NOT_CONFIGURED') {
          setProgress('warning', 'Falta activar el lector visual', 'El lector ya está preparado. Falta configurar de forma segura la clave del servicio de visión en Supabase.');
          return;
        }
