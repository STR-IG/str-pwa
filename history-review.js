(() => {
  ...
  function cleanWording() {
    document.querySelectorAll('#comparison-result-title,#analysis-ready-status,#comparison-progress-title').forEach((node) => {
      node.textContent = node.textContent.replace(/\bposibles?\s+(?=incidencia)/gi, '');
    });
  }
  const observer = new MutationObserver(cleanWording);
