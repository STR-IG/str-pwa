(() => {
  ...
  function classify(text) {
    ...
    const summaryMarkers = [/RESUMEN\s+DE\s+VARIABLES(?:\s+DEL\s+MES)?/, /PLUS\s+ROT[A-Z]*VIDAD/, /COMIDAS?\s+CAN\s+GUA?SCH/, /PLUS\s+NOCTURN/, /PLUS\s+(?:DE\s+)?TURNO/, /PLUS(?:ES)?\s+FESTIVO/, /DIETAS?\s+FESTIVOS?/, /PLUS(?:ES)?\s+VACACIONES?/];
    ...
    const dailyMarkers = [/\bFECHA\b/, /\bDIA\b/, /\bHORARIO\b/, /HORA\s+INICIO/, /HORA\s+FIN/, /HORAS?\s+TEOR/, /\bDIAS\b/].filter(re => re.test(t)).length;
    const dateRows = (t.match(/\b\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}\b/g) || []).length;
    return {
      looksLikeSummary: hasSummaryHeading || (knownVariables >= 3 && hasSupportBlock),
      includesDailyRegister: dailyMarkers >= 3 || dateRows >= 3
    };
  }

  function showInvalid(reason) {
    state = 'invalid';
    if (privacyScan) { privacyScan.hidden = false; privacyScan.className = 'privacy-scan blocked'; }
  ...
  function showChecking() {
    state = 'checking';
    if (privacyScan) { privacyScan.hidden = false; privacyScan.className = 'privacy-scan checking'; }
    if (privacyIcon) privacyIcon.textContent = '🔎';
  ...
  async function validateFile(file, myVersion) {
    if (!isTimesheetScreen() || !file) { state = 'idle'; return; }
    showChecking();
    try {
  ...
  confirmButton.addEventListener('click', (event) => {
    if (!isTimesheetScreen() || state === 'valid') return;
