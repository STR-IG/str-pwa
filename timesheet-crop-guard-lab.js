...
(() => {
  ...
  function parseVariables(block) {
    ...
    if (Object.keys(result).length === 0) warnings.push('Se encontró «Resumen de variables del mes», pero no se pudieron leer sus conceptos.');
  ...
  function extractFromText(rawText) {
    ...
    if (!block) {
      return { ok: false, code: 'SUMMARY_NOT_FOUND', message: 'No encuentro la tabla «Resumen de variables del mes». Usa un PDF donde esa tabla sea visible.', values: {}, sourceBlock: [] };
    }
    ...
    return {
      ...
      message: Object.keys(parsed.values).length > 0 ? 'Resumen de variables leído.' : parsed.warnings[0],
  ...
  window.STRIG_TIMESHEET_READER = { extractFromText, allowedVariables: ALLOWED_VARIABLES.map(v => v.key) };
})();
