...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "authorize") {
      return json(req, { authorized: true });
    }
    ...
    if (action === "upsert") {
      ...
      if (
        ...
        !employeeNumber || employeeNumber.length > 40 ||
        !/^\d{9}$/.test(phone)
      ) {
      ...
      if (error) return json(req, { error: "UPSERT_FAILED" }, 500);
    ...
    console.error("Unexpected manage-affiliates error", error);
