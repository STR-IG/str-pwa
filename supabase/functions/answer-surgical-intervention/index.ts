...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const flexibleDays = relationship.startsWith("Persona conviviente") || queryType === "intervencion_quirurgica_primer_grado" || queryType === "intervencion_quirurgica_segundo_grado";
    const validDayCount = flexibleDays ? leaveDays.length >= 1 && leaveDays.length <= 5 : leaveDays.length === 5;
    ...
    console.error("Unexpected answer-surgical-intervention error", error instanceof Error ? error.message : "unknown");
