...
function minutesBetween(start:string,end:string){const [sh,sm]=start.split(":").map(Number),[eh,em]=end.split(":").map(Number);let a=sh*60+sm,b=eh*60+em;if(b<a)b+=1440;return b-a}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(accessError)return json(req,{error:"AUTHORIZATION_CHECK_FAILED"},500);if(!access)return json(req,{error:"FORBIDDEN"},403);

    const b=await req.json().catch(()=>({}));
    const caseType=String(b?.caseType??""),duringWork=String(b?.duringWork??""),region=b?.region==null?null:String(b.region),doctorType=b?.doctorType==null?null:String(b.doctorType),relationship=b?.relationship==null?null:String(b.relationship),needsEscort=b?.needsEscort==null?null:String(b.needsEscort),dependency=b?.dependency==null?null:String(b.dependency),medicalProof=b?.medicalProof==null?null:String(b.medicalProof),visitDate=String(b?.visitDate??""),absenceStart=String(b?.absenceStart??""),absenceEnd=String(b?.absenceEnd??""),workShift=String(b?.workShift??"");
    const standard=caseType==="own"||caseType==="child",dependent=caseType==="dependent",occupational=caseType==="occupational";
    const common=caseTypes.has(caseType)&&duringWork==="yes"&&isDate(visitDate)&&isTime(absenceStart)&&isTime(absenceEnd)&&shifts.has(workShift);
    ...
    const requestedMinutes=minutesBetween(absenceStart,absenceEnd);
    if(requestedMinutes<=0||requestedMinutes>1440)return json(req,{error:"INVALID_TIME_RANGE"},400);

    const facts={caseType,duringWork,region,doctorType,relationship,needsEscort,dependency,medicalProof,visitDate,absenceStart,absenceEnd,requestedMinutes,workShift};
    const ai=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${openAiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-5.4-mini",store:false,reasoning:{effort:"low"},max_output_tokens:900,safety_identifier:await safetyIdentifier(user.id),instructions:`Eres un asistente informativo de STR-IG. Analiza datos estructurados sobre visita médica. IMPORTANTE: absenceStart y absenceEnd son las horas de INICIO Y FIN DE LA AUSENCIA SOLICITADA, no las horas de entrada/salida que consten en el justificante médico. PACTO INTERNO: artículo 18 del Pacto adicional de 29/07/1993, vigente según STSJ de Cataluña 3419/2025: visita médica propia durante jornada, máximo retribuido de 4 horas dentro de la comarca de residencia y 6 horas fuera; comprende Seguridad Social, médico de cabecera, especialista y médico particular. Para accidente de trabajo o enfermedad profesional, tiempo necesario. Para acompañar hijos/as menores de 16 años, cuatro salidas al año, cada una de 4 horas dentro de comarca o 6 fuera. Acuerdo de 19/09/2002: no se exige que el justificante médico incluya horas de entrada y salida de la consulta. Si el caso es own o child, compara requestedMinutes con 240 minutos dentro de comarca o 360 fuera y avisa claramente si el intervalo solicitado supera el máximo retribuido previsto, sin confundirlo con la duración real de la consulta. CONVENIO GENERAL DE LA INDUSTRIA QUÍMICA: tiempo indispensable para acompañar a consulta médica al cónyuge o familiar de primer grado que esté a cargo, si coincide con jornada. Para estar a cargo debe existir necesidad real de acompañamiento por edad, accidente o enfermedad que impida acudir solo; convivencia o situación similar de alta dependencia; y acreditación mediante certificación o documento oficial de facultativo. En caso de edad, la dependencia se entiende hasta los 18 años. No incluyas ni menciones el concepto específico de visita médica de familiar de primer grado con diversidad funcional/discapacidad: se gestiona en un permiso separado. Nunca apliques las 4/6 horas al supuesto dependent; ahí corresponde tiempo indispensable si se cumplen los requisitos. Si cualquiera de needsEscort, dependency o medicalProof es no, usa not_compatible o revision según proceda y explica qué requisito falta. No inventes diagnósticos, hechos ni duración concreta del tiempo indispensable. Responde en español, claro, prudente y breve.`,input:JSON.stringify(facts),text:{verbosity:"low",format:{type:"json_schema",name:"medical_visit_guidance",strict:true,schema:{type:"object",properties:{status:{type:"string",enum:["compatible","revision","not_compatible"]},title:{type:"string"},summary:{type:"string"},reasons:{type:"array",items:{type:"string"}},recommendations:{type:"array",items:{type:"string"}}},required:["status","title","summary","reasons","recommendations"],additionalProperties:false}}}})});
